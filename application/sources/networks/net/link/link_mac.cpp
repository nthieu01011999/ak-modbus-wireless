...
void fsm_link_mac_state_handle(ak_msg_t* msg) {
	...
	case AC_LINK_MAC_FRAME_MW24_NRF_SEND_DONE: {
		LINK_DBG_SIG("AC_LINK_MAC_FRAME_MW24_NRF_SEND_DONE\n");
		if (link_mac_send_state_get() == LINK_MAC_SEND_STATE_SENDING) {
			if (++link_mac_frame_send.header.fidx < link_mac_frame_send.header.fnum) {
				...
				task_post_common_msg(AC_LINK_ID, AC_LINK_MW24_NRF_SEND_DONE, (uint8_t*)&link_pdu_send_done, sizeof(uint32_t));
